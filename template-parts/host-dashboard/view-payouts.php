<?php
/**
 * Host Dashboard: Payouts View (Decoupled REST API Client)
 *
 * Theme template strictly handles layout and rendering.
 * All earnings data is securely fetched via WordPress REST API from the Plugin backend.
 *
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const res = await fetch('/wp-json/hhb/v1/host/payouts', {
        ...
        if ( ! data.has_properties ) {
            document.getElementById('hhb-payouts-wrapper').innerHTML = `
                <div style="background:#fff; padding:40px; text-align:center; border-radius:12px; border:1px solid #e2e8f0;">
                    <span class="material-symbols-outlined" style="font-size:48px; color:#cbd5e1; margin-bottom:16px;">account_balance_wallet</span>
                    <h3 style="margin:0; color:#0f172a;">No Earnings Yet</h3>
                    <p style="color:#64748b; margin-top:8px;">Add a property and get your first booking to start earning!</p>
                </div>
            `;
            return;
        }
        ...
        elLife.className = ''; elLife.style = 'margin: 4px 0 0; font-size: 24px; color: #0f172a; font-weight:700;';
        elLife.innerText = data.lifetime_earnings;

        const elUp = document.getElementById('payout-upcoming');
        elUp.className = ''; elUp.style = 'margin: 4px 0 0; font-size: 24px; color: #0f172a; font-weight:700;';
        ...
        if ( data.payout_history && data.payout_history.length > 0 ) {
            tbody.innerHTML = data.payout_history.map(row => `
                <tr style="border-bottom: 1px solid #e2e8f0; ${row.status === 'cancelled' ? 'opacity:0.6;' : ''}">
                    <td style="padding: 16px 24px; color: #0f172a; font-weight: 500;">
                        #${row.booking_id}<br>
                        <span style="font-size: 12px; color: ${row.status_color};">${row.status_label}</span>
                    </td>
                    <td style="padding: 16px 24px; color: #0f172a;">${row.customer_name}</td>
            ...
            tbody.innerHTML = `<tr><td colspan="6" style="padding: 24px; text-align: center; color: #64748b;">No payout history available yet.</td></tr>`;
        }

    } catch (err) {
        document.getElementById('hhb-payouts-wrapper').innerHTML = `<div style="padding:20px; background:#fee2e2; color:#991b1b; border-radius:8px;">Failed to load payouts data. Please try again later.</div>`;
    }
});
</script>
